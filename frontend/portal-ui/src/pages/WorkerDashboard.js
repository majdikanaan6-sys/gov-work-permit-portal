      ...
      return [
        {
          id: 1,
          message:
            "Your work permit application has been pre-authorized. Please complete IHC payment.",
          type: "warning",
        },
      ];
    case "IHC_REQUIRED":
      ...
      return [
        {
          id: 3,
          message:
            "Your work permit application is under review. Please wait for approval.",
          type: "info",
        },
      ];
    default:
...
const WorkerDashboard = () => {
  ...
  let data = location.state?.application || null;
  if (!data) {
  ...
  useEffect(() => {
    if (location.state?.refresh) {
      fetchApplication();
      // Clear refresh flag to avoid repeated fetches
      navigate(location.pathname, { replace: true });
    }
  }, [location.state, navigate]);

  // Notifications update when status changes
  useEffect(() => {
    ...
    <div className={`dashboard-container ${darkMode ? "dark" : ""}`}>
      ...
      {showProfile && (
        <div className="modal-overlay" onClick={() => setShowProfile(false)}>
          <div className="modal-card" onClick={(e) => e.stopPropagation()}>
            ...
            <h3>Profile</h3>
            <p>
              <strong>Name:</strong> {application.worker?.name}
            </p>
            <p>
              <strong>Passport:</strong> {application.worker?.passport}
            </p>
            <p>
              <strong>Permit ID:</strong> {application.application?.reference}
            </p>
            <p>
      ...
      <div className="info-grid">
        ...
        <div className="ihc-card">
          ...
          <p>
            <strong>Amount:</strong> {application.ihc?.fee ? `BHD ${application.ihc.fee}` : "N/A"}
          </p>
          ...
          <div className="ihc-actions">
            {application.ihc?.payment_status === "PAID" ? (
              ...
              <button
                ...
                onClick={() => navigate("/worker/payment", { state: application })}
              >
                Pay Now
              </button>
